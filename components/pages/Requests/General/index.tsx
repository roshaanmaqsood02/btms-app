...
import { dummyGeneral, type General } from "./constants";
import { AppDataTable } from "@/components/common/tableData";
import { TableCell, TableRow } from "@/components/ui/table";
import { StatusBadge } from "@/components/common/statusBadges";
import { TablePagination } from "@/components/common/pagination";

const TABLE_COLUMNS = ["ATT ID", "NAME", "DATES", "DETAIL", "HR APPROVAL"];
...
export default function General() {
  ...
  const tableData: General[] = dummyGeneral;
      ...
      <AppDataTable columns={TABLE_COLUMNS} isLoading={isLoading}>
        {currentItems.map((request) => (
          <TableRow key={request.attendence_id} className="hover:bg-gray-50/50">
            ...
            <TableCell className="font-medium">{request.detail}</TableCell>
