...
import { dummyWorkFromHome, type WorkFromHome } from "./constants";
...
export default function WorkFromHome() {
  ...
  const tableData: WorkFromHome[] = dummyWorkFromHome;
      ...
      <AppDataTable columns={TABLE_COLUMNS} isLoading={isLoading}>
        {currentItems.map((request) => (
          <TableRow key={request.attendence_id} className="hover:bg-gray-50/50">
            ...
            <TableCell>{request.wfhType}</TableCell>
            <TableCell className="font-medium">{request.totalDays}</TableCell>
            <TableCell className="font-medium">{request.dates}</TableCell>
            <TableCell>
